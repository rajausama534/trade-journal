...
export function EdgeWorkspace({ tab, accounts, trades }: WorkspaceProps) {
  ...
  if (tab === "News") return <section className="edge-workspace"><div className="edge-grid two"><Info icon={Newspaper} title="Economic calendar" value="Watchlist filtered" text="High-impact events relevant to the instruments you trade."/><Info icon={ShieldAlert} title="News guardrail" value="30m before / after" text="Execution can be blocked around selected events."/></div><article className={card}><div className="empty-state"><Newspaper/><h2>Live news feed connection required</h2><p>The UI is ready; connect a licensed economic-calendar API to populate real events and enforce blackout windows.</p></div></article></section>;
  ...
  if (tab === "AI Coach") return <section className="edge-workspace"><div className="edge-grid three"><Info icon={Sparkles} title="Performance" value={`${stats.pnl>=0?"+":""}$${stats.pnl.toFixed(0)}`} text="Current realized P&L."/><Info icon={Brain} title="Win rate" value={`${stats.winRate.toFixed(1)}%`} text="Across the selected trade data."/><Info icon={Target} title="Discipline score" value={`${stats.edge.toFixed(0)}/100`} text="Placeholder until full rule-event logging is active."/></div><article className={card}><div className="edge-title"><Brain/><div><h2>AI trading coach</h2><p>Ask questions against your journal, discipline and performance data.</p></div></div><div className="ai-box"><span>Suggested question</span><b>What pattern is hurting my results most?</b><p>Connect a server-side AI endpoint to generate evidence-based coaching from your own trades and notes.</p></div><div className="ai-input"><input placeholder="Ask about your trading..."/><button className="primary">Ask</button></div></article></section>;
